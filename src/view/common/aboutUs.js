...
const AboutUs = () => {
  ...
  const options = {
    ...
    headerRight: ({ tintColor }) => (
      <>
        {(Platform.OS == 'android' && !searcher) && !(config?.unifiedAds && stores?.length == 1) &&
          <Button icon data={{
            icon: { name: 'search', type: 'ionicons', color: tintColor, size: 22 },
            onPress: async () => {
              navigation.navigate('AboutUsSearch', { searcher: true })
            }
          }} noMargin />}
      </>
    ),
  ...
  const othersItems = config?.items?.others
    ?.filter((item) => item?.title && item?.description)
    ?.map((item) => {
      return { title: item?.title, onPress: () => { navigation.navigate('AboutUsView', { title: item?.title, description: item?.description }); } }
    });
          ...
          },
          {
            component:
              <List data={[
                item?.email && {
                  title: item?.email,
                  description: (Platform.OS == 'android' && 'Caso você tenha alguma dúvida, sugestão ou reclamação envie-nos um e-mail.'),
                ...
                {
                  title: item?.website,
                  description: (Platform.OS == 'android' && 'Se desejar ver mais informações, acesse nosso site.'),
                  color: { title: colors.primary },
                  delay: false,
                  chevron: false,
                ...
                {
                  title: 'Horários',
                  description: helper.getAboutUsTodayHours(item?.openingHours?.default),
                  subdescription: helper.getAboutUsTodayHours(item?.openingHours?.default),
                  collapsible: collapsibleHours == index,
                  onPress: () => { setCollapsibleHours(index != collapsibleHours ? index : null); LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut); }
                },
                ...
                collapsibleHours == index && {
                  component:
                    <>
                      ...
                      {item?.openingHours?.others?.map?.((openingHours, index) => {
                        return (
                          <List
                            key={index}
                            data={[
  ...
  return (
    ...
    <FlatList
      ...
      {...!(config?.unifiedAds && stores?.length == 1) && {
        ...
        ListEmptyComponent: <NoStoreYet text={search} loading={isFetching || loading} />,
        onEndReached: () => {
              ...
              data={[
                {
                  component:
                    (config?.icon?.light && config?.icon?.dark)
                      ? <View style={{ marginVertical: 15, justifyContent: 'center', alignItems: 'center', marginLeft: 5 }}>
                        <FastImage
                      ...
                      : <Icon name={'person-circle'} type={'ionicons'} color={colors.tertiary} size={74} style={{ marginVertical: Platform.OS == 'ios' ? 8 : 7.5, width: 67, alignSelf: 'center' }}></Icon>
                  ,
                  padding: false
                },
                config?.about && {
                  component:
                    <TouchableWithoutFeedback onPress={() => { setSeeMore(!seeMore); }}>
                      <Text style={[MediumFontSize(), { color: colors.text, marginVertical: 10 }]}>{(config?.about?.length > 120 && !seeMore) ? `${config?.about?.substring(0, 120)}...` : config?.about} {(config?.about?.length > 120 && !seeMore) && <Text style={[MediumFontSize(), { color: colors.primary, marginVertical: 10, fontWeight: 'bold' }]}>Ver mais</Text>}</Text>
          ...
          {!search && !searcher &&
            <>
              <List data={[
                ...
                config?.website && { title: config?.website, color: { title: colors.primary }, delay: false, chevron: false, onPress: () => { Linking.openURL(config.website); } }
              ]} marginTop={!(config?.unifiedAds && stores?.length == 1)} />

              <List data={[
                config?.items?.terms && { title: 'Termos de Uso', onPress: () => { navigation.navigate('AboutUsView', { title: 'Termos de Uso', description: config?.items?.terms }); } },
                config?.items?.privacy && { title: 'Política de Privacidade', onPress: () => { navigation.navigate('AboutUsView', { title: 'Política de Privacidade', description: config?.items?.privacy }); } }
              ]} />

              <List data={othersItems} />

              <View style={{ marginTop: 30 }}>
                <View style={{ alignItems: "center" }}>
                  <Text style={[DescriptionFontSize(), { color: colors.secondary }]}>© {new Date().getFullYear()} Todos os direitos reservados.</Text>
                  <Text style={[DescriptionFontSize(), { color: colors.secondary }]}>{config?.name ? config?.name : '...'}</Text>
                </View>
              </View>
            </>
          }
